...
describe("structs", () => {
	...
	test("composition", async () => {
		...
		const second = Step.make({
			title: "second",
			run: Effect.andThen(push, subWorkflow("second")).pipe(
				Effect.andThen(end),
			),
			inputs: [],
		});
...
describe("classes", () => {
	test("workflow works", async () => {
		...
		class Grandparent1 extends Step.Class<Grandparent1>()({
			title: "grandparent1",
			inputs: [],
			run: push,
		}) {}
		class Parent1 extends Step.Class<Parent1>()({
			title: "parent1",
			inputs: [Grandparent1],
			run: push,
		}) {}
		class Grandparent2 extends Step.Class<Grandparent2>()({
			title: "grandparent2",
			inputs: [],
			run: push,
		}) {}
		class Parent2 extends Step.Class<Parent2>()({
			title: "parent2",
			inputs: [Grandparent2],
			run: push,
		}) {}
		class Child extends Step.Class<Child>()({
			title: "child",
			inputs: [Parent1, Parent2],
